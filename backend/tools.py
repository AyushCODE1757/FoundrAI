...
def deploy_to_github(repo_name: str, files_json: str) -> str:
    ...
    try:
        ...
        # Give GitHub a moment to asynchronously initialize the repository and branch
        time.sleep(2)

        # Get the dynamically assigned default branch (main or master)
        default_branch = repo.default_branch
        main_ref = repo.get_git_ref(f"heads/{default_branch}")
        main_commit = repo.get_git_commit(main_ref.object.sha)
        base_tree = main_commit.tree

        # Push all files in one commit using Git Trees API
        elements = []
        for filename, content in files.items():
            elements.append(InputGitTreeElement(filename, '100644', 'blob', content=content))
