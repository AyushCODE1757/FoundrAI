...
def deploy_to_github(repo_name: str, files_json: str) -> str:
    """
    Create a private GitHub repo named after the startup and push boilerplate files.
    files_json is a JSON string of {filename: content}.
    Returns the repo HTML URL on success.

    FIX: The original used create_git_blob() which fails with 404 on empty repos
    because the Git Data API requires an existing tree/commit to reference.
    An empty repo (auto_init=False) has no commits, so create_git_blob() returns 404.

    The fix: use repo.create_file() (GitHub Contents API) which handles blob creation,
    tree updates, and commits internally. Works perfectly on empty repos.
    """
    github_token = os.getenv("GITHUB_TOKEN", "")
    if not github_token or github_token.startswith("your_"):
        return "[FALLBACK] GITHUB_TOKEN not set. Cannot deploy. Set GITHUB_TOKEN in .env."
    try:
        ...
        # Sanitize repo name: lowercase, no spaces, max 80 chars
        safe_name = re.sub(r"[^a-zA-Z0-9\-]", "-", repo_name.lower()).strip("-")[:80]
        safe_name = re.sub(r"-+", "-", safe_name)

        # ── Step 1: Create private repo WITHOUT auto_init ─────────────────
        # auto_init=False → empty repo, zero commits.
        # create_file() works on empty repos. create_git_blob() does NOT.
        try:
            repo = user.create_repo(
                safe_name,
                private=True,
                description="Generated by FoundrAI 2.0 — AI-validated startup codebase",
                auto_init=False,   # ← THE ONLY CHANGE from the original
            )
        except GithubException as e:
            if e.status == 422:    # repo already exists — reuse it
                repo = user.get_repo(safe_name)
            else:
                raise

        # ── Step 2: Push files one by one via Contents API ────────────────
        # repo.create_file() = PUT /repos/{owner}/{repo}/contents/{path}
        # Creates blob + tree entry + commit in a single API call.
        # First call also creates the "main" branch automatically.
        for i, (filename, content) in enumerate(files.items()):
            commit_msg = "🤖 Initial commit by FoundrAI 2.0" if i == 0 else f"Add {filename}"
            try:
                repo.create_file(
                    path=filename,
                    message=commit_msg,
                ...
                if e.status == 422:   # file already exists — update instead
